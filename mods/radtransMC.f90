...
CONTAINS
  ...
  subroutine radtrans_MCsim( j,nummatSegs,numParts,rodOrplanar,&
  ...
  integer  :: j,nummatSegs,numParts,matType(:),o,ntime,radtrans_int
  integer  :: pfnumcells
  ...
  do o=1,numParts !o-loop, through per particle
    ...
    elseif( sourceType=='intern') then
      position = s * rang()
      i        = internal_init_i(position,matLength,nummatSegs)
...
!print *,"fluxavemat2: ",fluxavemat2
  ...
  function internal_init_i( position,matLength,numArrSz )
  integer :: numArrSz
  real(8) :: position,matLength(:)
...
!! Radtrans and Woodcock funcs and subs
  ...
  subroutine adv_pos_col_flux( oldpos,newpos,fluxfaces,flux,pfnumcells,plotflux,&
                               pltflux,j,mu )
...
!print *
  ...
  subroutine col_fbflux( oldpos,newpos,fluxfaces,fflux,bflux,pfnumcells,plotflux,&
                         pltflux,j,mu,matType,matLength,nummatSegs )
  !tallies flux contribution in each material withing fluxface bins 
  integer :: pfnumcells,j,nummatSegs,matType(:)
...
!if(larpos==1.0d0) STOP
  ...
  use radMCvars,    only: radMCbinplot
  use radWoodvars,  only: radWoodbinplot
  use KLWoodvars,   only: KLWoodbinplot
  ...
  !for each method
  use genRealzvars,         only: numRealz
  use transport_sharedvars, only: trprofile_binnum
  real(8) :: reflect(:),transmit(:)
