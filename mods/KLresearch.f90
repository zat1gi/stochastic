...
CONTAINS
  ...
  subroutine KL_eigenvalue( numEigs,alpha,Ak,Eig,P,sigave,&
  ...
  use KLvars,       only: KLvarkept_tol,KLvarcalc,AllEig,Allgam,varmain
  integer :: numEigs,levsrefEig,numSlice,KLrnumRealz
  real(8),allocatable :: gam(:),alpha(:),Ak(:),Eig(:),xi(:,:),KLrxivals(:,:)
  ...
  subroutine KL_Correlation( Corropts,Corrnumpoints,numEigs,Eig,Ak,&
  ...
  real(8) :: Eig(:),Ak(:),alpha(:),lamc,sigave,CoExp,P(2)
  ...
  real(8) :: gam(:),alpha(:),Ak(:),Eig(:),xi(:,:),lamc,totLength(2)
  ...
  subroutine KL_Cochart( numEigs,numSlice,P,sigave,lamc,alpha,&
                         Ak,Eig,avePath,totLength,gam,pltCo,pltConumof,pltCowhich,&
                         CoExp )
