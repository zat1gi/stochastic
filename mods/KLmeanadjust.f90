...
CONTAINS
  ...
  subroutine KLadjustmean( sigave,Eig,numEigs,Ak,alpha,lamc,&
  ...
  do j=1,KLrnumRealz
    intsigave = intsigave + &
                KLrxi_integral(j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals,0d0,s)/KLrnumRealz/s
  enddo
...
!print *,"adjusting realization: ",j
      xr = KLrxi_point(j,numEigs,Eig,Ak,alpha,lamc,sigave,0d0,KLrxivals)
      xl = 0d0
      do
!print *,"about to find next point"
        xr = findnextpoint(j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals)
!print *,"found next point: ",xr
        areacont = KLrxi_integral(j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals,xl,xr)/KLrnumRealz/s

        xmid = KLrxi_point(j,numEigs,Eig,Ak,alpha,lamc,sigave,(xr+xl)/2d0,KLrxivals)
        if(xmid>0d0) then
  ...
  function findnextpoint(j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals)
  ...
  curx = xl
  curs = KLrxi_point(j,numEigs,Eig,Ak,alpha,lamc,sigave,curx,KLrxivals)
...
!print *,"inside1: curx: ",curx,"  step: ",step
    if(curs*olds<0d0) then
      curx = refinenextpoint(j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals,oldx,curx)
...
!print *,"inside4: curx: ",curx,"  step: ",step
  ...
  function refinenextpoint(j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals,oldx,curx)
  ...
  function KLrxi_point(j,numEigs,Eig,Ak,alpha,lamc,sigave,xpos,KLrxivals)
  ! Evaluates KL reconstructed realizations at a given point
  integer :: j,numEigs
  real(8) :: Eig(:),Ak(:),alpha(:),lamc,sigave,xpos
  ...
  function KLrxi_integral(j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals,xl,xr)
  ! This function integrates on KL reconstructed realizations from xl to xr
  integer :: j,numEigs
  real(8) :: Eig(:),Ak(:),alpha(:),lamc,sigave,xl,xr
