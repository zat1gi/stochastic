...
CONTAINS
  ...
  subroutine KLrgenrealz( sigave,Eig,numEigs,lamc,KLrx,binPDF,&
...
!if(curEig==3) print *,"curEig/xiterm:         ",cureig," ",xiterm
  ...
  neg='no'
  call KLr_negsearch( j,numEigs,Eig,lamc,sigave,KLrxivals,s,neg )
  ...
  do i=1,KLrnumpoints(2)  !create realization
    KLrxisig(i) = KLrxi_point(j,numEigs,Eig,lamc,sigave,KLrxi(i),KLrxivals)
  enddo
  ...
  subroutine KLr_negsearch( j,numEigs,Eig,lamc,sigave,KLrxivals,s,neg )
  use KLvars, only: alpha, Ak
  integer :: j,numEigs
  real(8) :: Eig(:),lamc,sigave,KLrxivals(:,:),s
  character(3) :: neg
  ...
  do i=1,numEigs
    ...
    minsig=KLrxi_point(j,numEigs,Eig,lamc,sigave,minpos,KLrxivals)
    do k=2,nminnersteps
      xpos=(outerstep*(i-1)+innerstep*(k-1))
      xsig= KLrxi_point(j,numEigs,Eig,lamc,sigave,xpos,KLrxivals)
      if(xsig<minsig) then
        minsig=xsig
        minpos=xpos
      endif
