...
CONTAINS
  ...
  subroutine KLrgenrealz( sigave,Eig,numEigs,Ak,alpha,gam,lamc,KLrx,binPDF,&
  ...
  real(8), dimension(numEigs) :: Eig,Ak,alpha,gam
...
!if(curEig==3) print *,"curEig/xiterm:         ",cureig," ",xiterm
  ...
  neg='no'
  call KLr_negsearch( j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals,s,neg )
  ...
  do i=1,KLrnumpoints(2)  !create realization
    KLrxisig(i) = KLrxi_point(j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxi(i),KLrxivals)
  enddo
  ...
  if( pltKLrrealz(1) .NE. 'noplot' ) then  !plot using generic plotter
    do m=1,pltKLrrealznumof
      ...
      if( pltKLrrealzPointorXi(m) .EQ. 'fxi' ) then  !create a realz, fixed xi
        ...
        KLrxisig = 0
        do i=1,KLrnumpoints(2)
          KLrxisig(i) = KLrxi_point(pltKLrrealzwhich(1,m),tnumEigs,Eig,&
                                    Ak,alpha,lamc,sigave,KLrxi(i),KLrxivals)
  ...
  subroutine KLr_negsearch( j,numEigs,Eig,Ak,alpha,lamc,sigave,KLrxivals,s,neg )
  integer :: j,numEigs
  real(8) :: Eig(:),Ak(:),alpha(:),lamc,sigave,KLrxivals(:,:),s
  character(3) :: neg
  ...
  do i=1,numEigs
    ...
    minsig=KLrxi_point(j,numEigs,Eig,Ak,alpha,lamc,sigave,minpos,KLrxivals)
    do k=2,nminnersteps
      xpos=(outerstep*(i-1)+innerstep*(k-1))
      xsig= KLrxi_point(j,numEigs,Eig,Ak,alpha,lamc,sigave,xpos,KLrxivals)
      if(xsig<minsig) then
        minsig=xsig
        minpos=xpos
      endif
