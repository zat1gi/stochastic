...
!print *,"rtnumParts,ttnumParts:",rtnumParts,ttnumParts
  ...
  if(flPCErefl)                    call solve_PCEQoIs( PCEcoefsrefl, samplePCExis(:,:,numPCEcells+1), &
                                                       stocMC_reflectionPCE, PCEreflsamples)
  if(flPCErefl.and. flPCEtran)     call solve_PCEQoIs( PCEcoefstran, samplePCExis(:,:,numPCEcells+2), &
                                                       stocMC_transmissionPCE, PCEtransamples)
  if(flPCErefl.and..not.flPCEtran) call solve_PCEQoIs( PCEcoefstran, samplePCExis(:,:,numPCEcells+1), &
                                                       stocMC_transmissionPCE, PCEtransamples)
  if(numPCEcells>0) then
    do i=1,numPCEcells
      call solve_PCEQoIs( PCEcoefscells(i,:), samplePCExis(:,:,i), &
                          stocMC_fluxallPCE(i,:), PCEcellssamples(i,:) )
    enddo
  endif
  ...
  subroutine solve_PCEQoIs(coefs,samplexis,moments,samples)
  ...
  real(8), intent(out):: moments(:),samples(:)
  integer :: isamp
  ...
  !It now does the same for leakage data generated by sampling from a PCE model.
  use genRealzvars, only: numRealz
  use MCvars,       only: reflect, transmit, chTrantype
  use UQvars, only: chUQtype,numPCEQoIsamps,PCEreflsamples,PCEtransamples
  real(8) :: smrefl,lgrefl,smtran,lgtran,boundbuff,smreflPCE,lgreflPCE,smtranPCE,lgtranPCE
  ...
  !find PCE-sampled reflection binning/plotting bounds
  smreflPCE = 1d0
  lgreflPCE = 0d0
  smreflPCE = min(smreflPCE,minval(reflect))
  lgreflPCE = max(lgreflPCE,maxval(reflect))
  boundbuff = (lgreflPCE-smreflPCE)/8d0
  smreflPCE = merge(smreflPCE-boundbuff,0d0,smreflPCE-boundbuff>0d0)
  lgreflPCE = merge(lgreflPCE+boundbuff,1d0,lgreflPCE+boundbuff<1d0)
  smreflPCE = smreflPCE - 0.0000001d0 !these to ensure binning works in case of opaque or transparent
  lgreflPCE = lgreflPCE + 0.0000001d0
  ...
  smtranPCE = 1d0
  lgtranPCE = 0d0
  smtranPCE = min(smtranPCE,minval(transmit))
  lgtranPCE = max(lgtranPCE,maxval(transmit))
  boundbuff = (lgtranPCE-smtranPCE)/8d0
  smtranPCE = merge(smtranPCE-boundbuff,0d0,smtranPCE-boundbuff>0d0)
  lgtranPCE = merge(lgtranPCE+boundbuff,1d0,lgtranPCE+boundbuff<1d0)
  smtranPCE = smtranPCE - 0.0000001d0 !these to ensure binning works in case of opaque or transparent
  lgtranPCE = lgtranPCE + 0.0000001d0
  ...
  call system("test -e plots/tranreflprofile/tranreflprofilePCE.txt && rm plots/tranreflprofile/tranreflprofilePCE.txt")

  !bin and print data
  if(chTrantype=='GaussKL' .and. chUQtype=='PCE') then
    call radtrans_bin( smreflPCE,lgreflPCE,smtranPCE,lgtranPCE, numPCEQoIsamps,PCEreflsamples,PCEtransamples )
    call system("mv tranreflprofile.txt tranreflprofilePCE.txt")
    call system("mv auxiliary/leakagepdfs/refltran_leakagevals.txt auxiliary/leakagepdfs/refltran_leakagevalsPCE.txt")  
  endif
  call radtrans_bin( smrefl,lgrefl,smtran,lgtran, numRealz,reflect,transmit ) 
  ...
  select case (chTrantype)
    ...
    case("GaussKL")
      call system("mv tranreflprofile.txt plots/tranreflprofile")
      if(chUQtype=='PCE') call system("mv tranreflprofilePCE.txt plots/tranreflprofile")
  end select
  ...
  subroutine radtrans_bin( smrefl,lgrefl,smtran,lgtran,  numsamples,reflect,transmit )
  ...
  call store_in_bins( smrefl,lgrefl,trprofile_binnum,reflcounts,reflbounds,reflect,numsamples )
  call store_in_bins( smtran,lgtran,trprofile_binnum,trancounts,tranbounds,transmit,numsamples )

  tranprob = real(trancounts,8)/numsamples
  reflprob = real(reflcounts,8)/numsamples
