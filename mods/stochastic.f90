program stochastic
  ...
  real(8),allocatable :: gam(:),alpha(:),Ak(:),Eig(:),xi(:,:),binPDF(:,:),binBounds(:)
  ...
  call readinputstoc(      binNumof,numEigs,numSlice,levsrefEig,&
                           ...
                           pltxiBins,pltxiBinsnumof,pltxiBinswhich,&
                           pltxiBinsgauss,pltKLrrealzPointorXi,&
                           pltEigf,pltEigfnumof,pltEigfwhich,&
                           ...
                           pltCo,pltConumof,pltCowhich,plotmatdxs,&
                           Corrnumpoints,Corropts,radWood,KLWood,allowneg,&
                           distneg,plotflux,pfnumcells,pltflux,sourceType,seed )

  call testinputstoc(      pltEigfnumof,pltEigfwhich,numEigs,pltxiBinsnumof,&
                           pltxiBinswhich,binNumof,KLrnumRealz,KLrprintat,&
                           pltKLrrealznumof,pltKLrrealzwhich,pltEigf,pltxiBins,&
                           pltKLrrealz,trannprt,KLres,KLrec,radWood,&
                           pltgenrealz,pltgenrealznumof,pltgenrealzwhich,&
                           pltCo,pltConumof,pltCowhich,radMC,pltKLrrealzPointorXi,&
  ...
  if(KLres=='yes')   call KL_eigenvalue( numEigs,alpha,Ak,Eig,P,sigave,&
                           levsrefEig,lamc,numSlice,gam,xi,pltEigf,&
                           pltEigfnumof,pltEigfwhich,KLrxivals,KLrnumRealz )
  if(KLres=='yes')   call KL_Correlation( Corropts,Corrnumpoints,numEigs,Eig,Ak,&
                           alpha,lamc,sigave,CoExp,P )
  ...
  do j=1,numRealz
    ...
    if(radWood=='yes') call WoodcockMC( j,matType,matLength,nummatSegs,&
                           ...
                           KLWood_rej,Eig,Ak,alpha,sigave,KLrxivals,rodOrplanar,&
    ...
    if(KLres=='yes') call KL_collect( nummatSegs,matLength,matType,j,xi,&
                           numEigs,gam,alpha,Ak,sigave,Eig,lamc,totLength,&
                           time,ntime )
  ...
  if(KLres=='yes') call KL_Cochart( numEigs,numSlice,P,sigave,lamc,&
                           alpha,Ak,Eig,avePath,totLength,gam,pltCo,pltConumof,&
                           pltCowhich,CoExp )
  ...
  do j=1,KLrnumRealz
    if(KLrec=='yes') call KLrgenrealz( sigave,Eig,numEigs,Ak,alpha,gam,lamc,KLrx,&
    ...
    if(mod(j,KLrprintat)==0 .AND. KLrec=='yes') call KLr_time( time,ntime,j,&
                           KLrnumRealz,t1)
  enddo
  if(KLadjust=='yes') call KLadjustmean( sigave,Eig,numEigs,Ak,alpha,lamc,&
                           KLrnumRealz,KLrxivals )
  if(KLrec=='yes') call KLreval( KLrnumpoints,pltKLrrealznumof,pltKLrrealzarray,&
                           pltKLrrealz,KLrrandarray,Eig,Ak,alpha,gam,lamc,&
